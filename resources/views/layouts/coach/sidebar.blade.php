<div class="pb-3 sidebar pe-4">
    <nav class="navbar bg-light navbar-light">
        @php
            $competitionOpen = Route::is('coach.competition-details');
            $coachUser = Auth::guard('coach')->user();
        @endphp
        <a href="{{ route('coach.dashboard') }}" class="mx-4 mb-3 navbar-brand">
            <img src="{{ asset('assets/images/logo.png') }}" class="img-fluid" alt="">
        </a>
        <div class="mb-4 d-flex align-items-center ms-4">
            <div class="position-relative">
                <img class="rounded-circle" src="{{ $coachUser->image ?? asset('assets/images/user.jpg') }}" alt=""
                    style="width: 40px; height: 40px;">
                <div class="bottom-0 p-1 border border-2 border-white bg-success rounded-circle position-absolute end-0">
                </div>
            </div>
            <div class="ms-3">
                <h6 class="mb-0 text-truncate" style="max-width: 150px;">{{ $coachUser->name ?? 'Coach' }}</h6>
                <span>Coach</span>
            </div>
        </div>
        <div class="navbar-nav w-100 sidebar-small">
            <a href="{{ route('coach.dashboard') }}" class="nav-item nav-link {{ Route::is('coach.dashboard') ? 'active' : '' }}">
            ...
            <div class="sidebar-section">
                <button type="button"
                    ...
                    data-bs-target="#coachCompetitions"
                    aria-expanded="{{ $competitionOpen ? 'true' : 'false' }}"
                    aria-controls="coachCompetitions">
                ...
                <div id="coachCompetitions" class="sidebar-submenu collapse {{ $competitionOpen ? 'show' : '' }}">
                    <a href="{{ route('coach.competition-details') }}" class="nav-item nav-link sidebar-subnav-link {{ Route::is('coach.competition-details') ? 'active' : '' }}">
