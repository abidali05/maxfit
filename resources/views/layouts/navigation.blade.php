...
<nav class="navbar navbar-expand bg-light navbar-light sticky-top px-4 py-0">
    @php
        if (Auth::guard('coach')->check()) {
            $navUser = Auth::guard('coach')->user();
            $logoutRoute = route('coach.logout');
        } elseif (Auth::guard('branch')->check()) {
            $navUser = Auth::guard('branch')->user();
            $logoutRoute = route('branch.logout');
        } else {
            $navUser = Auth::user();
    ...
    <form class="d-none d-md-flex ms-4 ">
        <input class="form-control border-0 d-none" type="search" placeholder="Search">
    </form>
    <div class="navbar-nav align-items-center ms-auto">
        ...
        <div class="nav-item dropdown">
            <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown">
                <img class="rounded-circle me-lg-2" src="{{ $navUser->image ?? asset('assets/images/user.jpg') }}" alt=""
                    style="width: 40px; height: 40px;">
                <span class="d-none d-lg-inline-flex">{{ $navUser->name ?? '' }}</span>
            </a>
            <div class="dropdown-menu dropdown-menu-end bg-light border-0 rounded-0 rounded-bottom m-0">
                @if (Auth::guard('coach')->check())
