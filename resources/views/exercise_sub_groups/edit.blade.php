@extends('layouts.app')
@section('title', 'Edit Sub-Group: ' . $subGroup->name)
@section('content')
    <div class="container-fluid pt-4 px-4" style="min-height: 82.5vh">
        <form action="{{ route('exercise-sub-groups.update', $subGroup->id) }}" method="POST" enctype="multipart/form-data" id="subGroupForm">
            @csrf
            @method('PUT')
                ...
                <div class="col-lg-4">
                    <div class="bg-light rounded p-4 h-100 shadow-sm">
                        <div class="d-flex align-items-center justify-content-between mb-4 border-bottom pb-2">
                            <div>
                                <h5 class="mb-0 fw-bold">Edit Sub-Group</h5>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $subGroup->name) }}" placeholder="e.g. Batsman, Bowler" required>
                            @error('name')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                        </div>

                        <div class="mb-3">
                            <label for="sub_title" class="form-label fw-bold">Subtitle / Tagline</label>
                            <input type="text" name="sub_title" id="sub_title" class="form-control" value="{{ old('sub_title', $subGroup->sub_title) }}" placeholder="e.g. Batting Specific Drills & Agility">
                        ...
                        <div class="mb-3">
                            ...
                            @if($subGroup->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $subGroup->image) }}" alt="" class="rounded img-thumbnail" style="max-height: 80px;">
                                </div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-select">
                                <option value="active" {{ old('status', $subGroup->status) === 'active' ? 'selected' : '' }}>Active</option>
                                <option value="inactive" {{ old('status', $subGroup->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('status')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                        </div>

                        <div class="mt-4 pt-3 border-top d-grid">
                            <button type="submit" class="btn btn-primary"><i class="fa fa-save me-2"></i>Update Sub-Group & Exercises</button>
                        </div>
                    </div>
                </div>
