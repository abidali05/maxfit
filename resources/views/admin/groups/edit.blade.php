...
@section('content')
    <div class="container-fluid pt-4 px-4" style="min-height: 82.5vh">
        <form action="{{ route('groups.update', $group->id) }}" method="POST" id="groupForm">
            @csrf
            @method('PUT')
            <div class="row g-4">
                <div class="col-12">
                    <div class="bg-light rounded p-4 shadow-sm">
                        <div class="d-flex align-items-center justify-content-between mb-4 border-bottom pb-2">
                            <div>
                                <h5 class="mb-0 fw-bold text-dark"><i class="fa fa-users text-primary me-2"></i>Edit Athlete Group</h5>
                                <small class="text-muted">Modify group details, assigned coach, criteria, and athlete invitations.</small>
                            </div>
                            <a href="{{ route('groups.show', $group->id) }}" class="btn btn-outline-secondary btn-sm"><i class="fa fa-arrow-left me-2"></i>Back to Group</a>
                        </div>
                            ...
                            <div class="col-md-6 col-lg-4">
                                ...
                                <select class="form-select select2" name="coach_id" id="coach_id" required>
                                    ...
                                    @foreach($coaches as $coach)
                                        <option value="{{ $coach->id }}" {{ old('coach_id', $group->coach_id) == $coach->id ? 'selected' : '' }}>{{ $coach->name }}</option>
                                    @endforeach
                                </select>
